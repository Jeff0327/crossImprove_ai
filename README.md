...
The toy solver is already correct on arithmetic, so nothing *improves* and the gate honestly keeps the parent — that's the correct no-noise-promotion behavior, not a bug. For genuine capability gain, plug `LLMMutator(llm)` (in `agent/genome.py`) and give `Agent` an `llm` exposing `.complete(prompt) -> str`; that single seam fails closed until wired. <br>토이 솔버는 산술에서 이미 정답이라 *개선이 없고* 게이트가 부모를 유지한다 — 버그가 아니라 노이즈를 승급 안 하는 올바른 동작이다. 진짜 능력 향상은 `agent/genome.py`의 `LLMMutator(llm)`를 끼우고 `Agent`에 `.complete(prompt) -> str` llm을 주면 된다. 이 seam은 연결 전까지 fail-closed다.
