...
```bash
# the full loop runs end-to-end WITHOUT an LLM via a deterministic ToyMutator,
# so you can exercise archive + diversity sampling + measured gate + role-swap.
# 전체 루프는 ToyMutator로 LLM 없이 끝까지 돈다 (아카이브·다양성 샘플링·실측 게이트·역할교대 검증용).
python3 -m orchestrator.loop
```

The toy genome drives behavior: the two seed agents start with a buggy "naive" strategy and a mutation fixes it to "correct" — a real measured improvement, so the loop actually **promotes** at generation 0 and then converges (no further noise promotions). The full mutate→measure→gate→commit→archive cycle runs without an LLM. For genuine capability gain, plug `LLMMutator(llm)` (in `agent/genome.py`) and give `Agent` an `llm` exposing `.complete(prompt) -> str`; that single seam fails closed until wired. <br>토이 genome이 행동을 구동한다: 두 시드 에이전트는 버그 있는 "naive" 전략으로 시작하고 변이가 이를 "correct"로 고친다 — 실제 측정된 개선이라, 루프가 0세대에서 **승급**한 뒤 수렴한다(이후 노이즈 승급 없음). mutate→measure→gate→commit→archive 전체 사이클이 LLM 없이 돈다. 진짜 능력 향상은 `agent/genome.py`의 `LLMMutator(llm)`를 끼우고 `Agent`에 `.complete(prompt) -> str` llm을 주면 된다. 이 seam은 연결 전까지 fail-closed다.
...
```
agent/        genome — the loop MAY edit these (agent, genome, debate, types) 게놈(편집 가능)
...
**Integrated skeleton.** The full loop runs end-to-end without an LLM (ToyMutator) and *demonstrates a real promotion*: a genome mutation fixes a buggy seed strategy, the measured paired scores improve, the gate promotes, then the loop converges. It exercises archive + softmax diversity sampling, role-swap via a unified `Agent`, and rc-checked commits that refuse empty (fake) promotions. The sandbox applies POSIX rlimits + a stripped env + an fd-based verdict channel (anti-spoof) and best-effort process-group reaping. 53 tests cover all of it.

**One remaining seam:** genuine capability gain requires `LLMMutator` rewriting agent code on a real domain — it fails closed until wired. The sandbox hardening is **real but partial**: it does NOT isolate filesystem or network. Process-group reaping is **best-effort** — a verifier that spawns and exits faster than we can signal can race a grandchild out; established grandchildren are reliably reaped. **Run untrusted self-generated code only inside a container/cgroup (no host mounts, egress off);** a subprocess + rlimits is not a complete boundary.

**통합된 골격.** 전체 루프가 LLM 없이 끝까지 돌며 *실제 승급을 시연*한다: genome 변이가 버그 시드 전략을 고치고, 측정된 paired 점수가 오르고, 게이트가 승급한 뒤 수렴한다. 아카이브 + softmax 다양성 샘플링, 통합 `Agent` 역할 교대, 빈(가짜) 승급을 거부하는 반환코드 검사 커밋을 수행한다. 샌드박스는 POSIX rlimit + env strip + fd 판결 채널(스푸핑 방지) + best-effort 프로세스그룹 회수를 적용한다. 53개 테스트가 이 전부를 덮는다.

**남은 seam 하나:** 진짜 능력 향상은 `LLMMutator`가 실제 도메인에서 agent 코드를 재작성해야 하며, 연결 전까지 fail-closed다. 샌드박스 강화는 **실재하지만 부분적**이다 — 파일시스템·네트워크는 격리하지 않는다. 프로세스그룹 회수는 **best-effort**다 — 시그널보다 빨리 spawn-and-exit하면 손자가 레이스로 빠질 수 있고, 자리잡은 손자는 확실히 회수된다. **자가 생성 코드는 컨테이너/cgroup 안(호스트 마운트 없음, egress 차단)에서만 돌려라.**
...
```bash
pip install pytest
pytest -q          # 53 behavioral tests, stdlib only, no LLM/network
```
...
이 스위트(53개)는 임포트가 아니라 *설계 의도*를 검증한다: 절차적 생성의 신선도, 적대적
